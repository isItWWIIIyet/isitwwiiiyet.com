$(document).ready(function () {
    (function () {
        ...
        $('#modal-contactSupport input[type=submit]').on('click', function (ev) {
            ...
            $('#modal-contactSupport #nameInput').parent().toggleClass('has-danger', false)
            $('#modal-contactSupport #emailInput').parent().toggleClass('has-danger', false)
            $('#modal-contactSupport #subjectInput').parent().toggleClass('has-danger', false)
            $('#modal-contactSupport #descriptionInput').parent().toggleClass('has-danger', false)
            $('#modal-contactSupport .drilldown').parent().parent().toggleClass('has-danger', false)
            var error = false;
            try {
                var name = $('#nameInput').val();
                if (name.trim().length == 0) { $('#nameInput').parent().toggleClass('has-danger', true); error = true; }

                var email = $('#emailInput').val();
                if (email.trim().length == 0) { $('#emailInput').parent().toggleClass('has-danger', true); error = true; }

                var subject = $('#subjectInput').val();
                if (subject.trim().length == 0) { $('#subjectInput').parent().toggleClass('has-danger', true); error = true; }

                var description = $('#descriptionInput').val();
                if (description.trim().length == 0) { $('#descriptionInput').parent().toggleClass('has-danger', true); error = true; }
                ...
                if (type.trim().length == 0) { $(drillDowns[drillDowns.length - 1]).parent().parent().toggleClass('has-danger', true); error = true; }
